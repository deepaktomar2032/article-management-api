...
describe('ArticleService', () => {
   ...
   describe('createArticle Function', () => {
      it('should create a new article if author exists', async () => {
         ...
         const insertEntrySpy = jest.spyOn(mockArticleAdapter, 'insertEntry').mockResolvedValue(articleId)
         ...
         expect(mockArticleAdapter.insertEntry).toHaveBeenCalledWith(expect.objectContaining({ authorId, title, content }))

         expect(findEntrySpy).toHaveBeenCalledTimes(1)
         expect(insertEntrySpy).toHaveBeenCalledTimes(1)
         expect(result).toEqual({ successful: true, message: message.Article_Created_Successfully, articleId })
      })
      ...
      it('should not create a new article if author not found', async () => {
         ...
         const insertEntrySpy = jest.spyOn(mockArticleAdapter, 'insertEntry').mockResolvedValue(article)
   ...
   describe('getArticles', () => {
      it('should respond with a list of all articles available in database', async () => {
         const article = { id, authorId, title, content, createdAt }
         const findEntriesSpy = jest.spyOn(mockArticleAdapter, 'findEntries').mockResolvedValue([article])
